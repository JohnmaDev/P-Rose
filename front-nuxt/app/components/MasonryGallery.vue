<template>
  ...
  <section id="cortes" class="mt-24 w-full scroll-mt-24">
    ...
    <div v-if="!pending && cuts.length > 0" class="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 w-full px-2 sm:px-0">
      <div
        v-for="cut in cuts"
        :key="cut.id"
    ...
    <div v-else-if="pending" class="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 w-full">
...
// useFetch con SSR — las fotos de cortes son visibles en el HTML para Google
const { data, pending } = await useFetch<{ ok: boolean; cuts: Cut[] }>('/api/get_cuts', {
  key: 'gallery-cuts',
  default: () => ({ ok: false, cuts: [] }),
})

const cuts = computed(() => data.value?.cuts || [])
</script>
